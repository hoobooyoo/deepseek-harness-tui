...
import {LOGO_LEFT, LOGO_MARKS, LOGO_RIGHT} from './logo.js';
import {deriveSessionSearchResults} from './session-search.js';
import {BRACKETED_PASTE_DISABLE, BRACKETED_PASTE_ENABLE, consumeBracketedPaste, isBracketedPasteStartPrefix, normalizePasteText, takePlainTextRun,} from './terminal-input.js';
import {PALETTE} from './theme.js';

const C = PALETTE;

/**
 * Accent color for each agent mode (opencode: plan = highlight peach, goal =
 * red).
 */
const MODE_COLORS = {
  plan: C.accent,
...
/** Debug log — writes to /tmp/deepcode-debug.log */
const DEBUG_LOG = '/tmp/deepcode-debug.log';
function dlog(msg) {
  try {
    appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] [UI] ${msg}\n`);
  } catch {
  }
}
...
/**
 * opencode's home prompt max width (tui config `prompt.max_width`, default
 * 75).
 */
const PROMPT_MAX_WIDTH = 75;
/**
 * Fixed top status area, matching grok-build's agent view: one blank row of
 * outer padding, the single status row, one gap row before the conversation
 * (blank / status / blank). On short terminals (≤ SHORT_TERMINAL_ROWS) the
 * padding and gap are dropped and the status row sits at the very top.
 */
const TOP_BAR_HEIGHT = 3;
/** grok-build `SHORT_TERMINAL_ROWS`: at or below this height the top area's
 * padding/gap rows are dropped. */
const SHORT_TERMINAL_ROWS = 16;
/** Effective top area height: the padding/gap rows collapse on short
 * terminals (grok-build drops `outer_vpad`/`status_gap` there). */
function topBarHeight(rows) {
  return rows > SHORT_TERMINAL_ROWS ? TOP_BAR_HEIGHT : 1;
}
/** Max option rows shown in a picker (fixed height; excess scrolls). */
const PICKER_MAX_ROWS = 8;
/** grok-build `MAX_QUEUE_HEIGHT`: at most this many queued-message rows
 * above the prompt box; the rest collapse into an "↑ N more" row. */
const MAX_QUEUE_ROWS = 3;
/** Session picker page size — matches the web sidebar's 5-session collapse. */
const SESSION_PAGE_SIZE = 5;
/**
 * Fixed number of physical input rows (Ctrl+Enter inserts a line break; older
 * content scrolls out).
 */
...
/**
 * Strip ANSI escape sequences (CSI SGR and related) from text so that
 * `displayWidth` and terminal rendering agree on column positions. Tool
 * output frequently carries colour codes (\u001b[31m, \u001b[1m, …) that
 * are invisible in the terminal but inflate display-width measurements
 * and cause panel backgrounds to bleed across the conversation boundary.
 * This mirrors what opencode does internally — raw ANSI is never
 * displayed; only the app's own semantic highlight codes paint the text.
 */
function stripAnsi(text) {
  return String(text ?? '').replace(/\u001b\[[\d;]*[a-zA-Z~$]/g, '');
}
...
let clipDaemon = null;
const CLIP_DAEMON =
    fileURLToPath(new URL('./x11-clipboard.py', import.meta.url));
const CLIP_READER =
    fileURLToPath(new URL('./x11-clipboard-read.py', import.meta.url));
...
function ensureClipDaemon() {
  ...
  try {
    const child =
        spawn('python3', [CLIP_DAEMON], {stdio: ['pipe', 'ignore', 'ignore']});
    child.stdin.on('error', () => {/* daemon died — next copy respawns */});
    child.once('error', () => {
      if (clipDaemon === child) clipDaemon = null;
    });
    child.once('exit', () => {
      if (clipDaemon === child) clipDaemon = null;
    });
...
function copyToClipboard(text) {
  ...
  if (canUseGdkClipboard()) {
    ...
    if (daemon !== null) {
      try {
        daemon.stdin.write(
            `SET ${Buffer.from(content, 'utf8').toString('base64')}\n`);
      } catch {
  ...
  const candidates = process.platform === 'win32' ? ['clip'] :
  ...
  const tryTool = (i) => {
    ...
    try {
      const child =
          spawn(candidates[i], [], {stdio: ['pipe', 'ignore', 'ignore']});
...
function readClipboard(cb) {
  if (canUseGdkClipboard()) {
    try {
      const child = spawn(
          'python3', [CLIP_READER], {stdio: ['ignore', 'pipe', 'ignore']});
      let out = '';
...
/**
 * Paste shortcut label for the current platform (⌘V on macOS, Ctrl+V
 * elsewhere).
 */
...
function tint(base, fgHex, ratio) {
  ...
  const ch = (i) => mix(parseInt(base.slice(i, i + 2), 16),
                        parseInt(fgHex.slice(i, i + 2), 16))
                        .toString(16)
                        .padStart(2, '0');
...
function renderLogoLine(line, color, bold) {
  ...
  for (const ch of line) {
    if (ch === '_')
      out += `${bg(shadow)} ${RESET}${bg(C.background)}`;
    else if (ch === '^')
      out += `${bg(shadow)}${fg(color)}▀${RESET}${bg(C.background)}`;
    else if (ch === '~')
      out += `${fg(shadow)}▀`;
    else if (ch === ',')
      out += `${fg(shadow)}▄`;
...
function renderLogo(width) {
  ...
  const halves = LOGO_LEFT.map(
      (l, i) => displayWidth(l) + 1 + displayWidth(LOGO_RIGHT[i]));
  ...
  for (let i = 0; i < LOGO_LEFT.length; i += 1) {
    ...
    lines.push(
        bg(C.background) + ' '.repeat(pad) + left + bg(C.background) + ' ' +
...
function displayWidth(s) {
  ...
  for (const ch of String(s)) {
    ...
    if (code === 0x09) {
      w += 4;
      continue;
    }  // tab → 4 columns (consistent with wrap's expansion)
    ...
    const zero = (code >= 0x0300 && code <= 0x036f) ||
        (code >= 0x1ab0 && code <= 0x1aff) ||
        (code >= 0x1dc0 && code <= 0x1dff) ||
        (code >= 0x20d0 && code <= 0x20ff) ||
        (code >= 0x200b && code <= 0x200f) ||
        (code >= 0x2060 && code <= 0x206f) ||
        (code >= 0xfe00 && code <= 0xfe2f) || code === 0xfeff ||
        code === 0x00ad;
    if (zero) continue;
    const wide = (code >= 0x1100 && code <= 0x115f) || code === 0x2329 ||
        code === 0x232a || (code >= 0x2e80 && code <= 0xa4cf) ||
 ...
 * would under-run the row and push the scrollbar column out of alignment.
...
function emojiPresentation(code) {
  return code === 0x231a || code === 0x231b ||
      (code >= 0x23e9 && code <= 0x23ec) || code === 0x23f0 ||
      code === 0x23f3 || (code >= 0x25fd && code <= 0x25fe) ||
      (code >= 0x2614 && code <= 0x2615) ||
      (code >= 0x2648 && code <= 0x2653) || code === 0x267f ||
      code === 0x2693 || code === 0x26a1 ||
      (code >= 0x26aa && code <= 0x26ab) ||
      (code >= 0x26bd && code <= 0x26be) ||
      (code >= 0x26c4 && code <= 0x26c5) || code === 0x26ce ||
      code === 0x26d4 || code === 0x26ea ||
      (code >= 0x26f2 && code <= 0x26f3) || code === 0x26f5 ||
      code === 0x26fa || code === 0x26fd || code === 0x2705 ||
      (code >= 0x270a && code <= 0x270b) || code === 0x2728 ||
      code === 0x274c || code === 0x274e ||
      (code >= 0x2753 && code <= 0x2755) || code === 0x2757 ||
      (code >= 0x2795 && code <= 0x2797) || code === 0x27b0 ||
      code === 0x27bf || (code >= 0x2b1b && code <= 0x2b1c) ||
...
function wrap(text, width) {
  ...
  // column here would expand to 8+ columns in the terminal, pushing the row
  // past the conversation block's edge.
...
function paintRow(bgColor, width, segments, padColor, reverse = false) {
  ...
  for (const seg of segments) {
    ...
    out += fg(seg.fg) + (seg.bold ? '\u001b[1m' : '') +
        (seg.italic ? '\u001b[3m' : '') + (reverse ? '\u001b[7m' : '') + text +
        (reverse ? '\u001b[27m' : '') + (seg.italic ? '\u001b[23m' : '') +
        (seg.bold ? '\u001b[22m' : '');
    col += w;
  }
  if (col < width) {
    out += fg(padColor ?? undefined) + (reverse ? '\u001b[7m' : '') +
        ' '.repeat(width - col) + (reverse ? '\u001b[27m' : '');
  }
  return out + RESET;
}
...
/**
 * Compact token count, web parity: 517 / 12.2K / 517K / 1.2M (one decimal
 * under three digits).
 */
...
function billedInputTokens(stats) {
  return (stats.inputTokens ?? 0) + (stats.cacheReadTokens ?? 0) +
 ...
 * prompt size over the recorded capacity.
 */
function fmtCtx(stats) {
  if (!stats.contextWindow || stats.contextWindow <= 0) return '';
  const used = stats.contextTokens ?? 0;
  return `${fmtTokens(used)}/${fmtTokens(stats.contextWindow)}`;
}

/**
 * Render the full-width top status row (grok-build top area): the compact
 * metrics (cache / in / out / ctx) aligned right with two columns of margin
 * off the window edge; the session title itself rides on the terminal window
 * title. While the agent is running the waiting spinner sits at the left.
 */
function topBarRow(stats, width, running = false, spinner = '') {
  const left = running ? `  ${spinner}` : '';
  const billed = billedInputTokens(stats);
  const metrics = [
    `cache ${billed > 0 ? fmtCacheHit(stats) : '—'}`,
    `in ${fmtTokens(billed)}`,
    `out ${fmtTokens(stats.outputTokens ?? 0)}`,
    `ctx ${fmtCtx(stats) || '—'}`,
  ].join('  ');
  const right = fit(metrics, Math.max(10, width - displayWidth(left) - 4));
  const gap =
      Math.max(1, width - displayWidth(left) - displayWidth(right) - 2);
  return paintRow(
      C.background, width,
      [
        {text: left, fg: C.text, bold: true},
        {text: ' '.repeat(gap), fg: C.background},
        {text: right, fg: C.muted},
        {text: '  ', fg: C.background},
      ],
      C.background);
}
...
function renderMarkdown(text, width) {
  const rows = [];
  const pushRow = (bgColor, segments) => rows.push(
      {bg: bgColor, segments, text: segments.map((s) => s.text).join('')});
  /**
  ...
  const pushPrefixed = (prefixSegs, contentSegs) => {
    const prefixW =
        prefixSegs.reduce((sum, s) => sum + displayWidth(s.text), 0);
    ...
    lines.forEach((line, i) => {
      if (i === 0)
        pushRow(C.background, [...prefixSegs, ...line]);
      else
        pushRow(
            C.background, [{text: ' '.repeat(prefixW), fg: C.text}, ...line]);
    });
  };
  ...
  for (const raw of lines) {
    ...
    if (fence) {
      ...
      if (fence[1] !== '')
        pushRow(
            C.backgroundElement,
            [{text: `  ${fence[1]}`, fg: C.muted, italic: true}]);
      continue;
    }
    if (raw.trim() === '') {
    ...
    if (heading) {
      ...
      pushPrefixed(
          ...
          renderInline(heading[2])
              .map((s) => ({...s, bold: true, fg: deep ? C.muted : s.fg})),
      );
      continue;
    }
    ...
    if (quote) {
      pushPrefixed(
          [{text: '  │ ', fg: C.muted}],
          renderInline(quote[1]).map(
              (s) => ({...s, italic: true, fg: C.muted})),
      );
      continue;
    }
    ...
    for (const line of wrapSegments(renderInline(raw), width - 2)) {
      pushRow(
          C.background,
          line.map((s, i) => (i === 0 ? {...s, text: ' ' + s.text} : s)));
    }
  }
  return rows;
}
...
/**
 * Argument keys whose value becomes a collapsed row's summary, per tool —
 * the web's `SUMMARY_KEYS` map (`dsh-client-ui-tool`). The order matters: the
...
/** Tools whose row is a `code` variant, i.e. the web's `TOOL_VARIANTS` map. */
const CODE_TOOLS = new Set(['run_code']);

/**
 * Tools whose settled, healthy row drops the state glyph and "· done" label,
 * mirroring the web's neutral card title (`Bash`/`Code`). Every other tool
 * keeps its glyph + label so a running or failed call is never ambiguous.
 */
const NEUTRAL_ROW_TOOLS = new Set(['bash', 'run_code']);

/** Display title per tool row — the web's `VARIANT_TITLE_KEYS` copy. */
function toolRowTitle(name) {
  if (name === 'bash') return 'Bash';
  if (CODE_TOOLS.has(name)) return 'Code';
  return name;
}

/** Tools whose summary is a workspace path, so it can be shortened for display. */
const PATH_SUMMARY_TOOLS = new Set(['read', 'write', 'edit']);

/**
 * Strip the workspace root from an absolute path, so a row shows
 * `lib/app.js` rather than `/home/you/project/lib/app.js` — the web's
 * `relativizeToCwd`. A path outside the workspace is returned unchanged.
 * @param text - the candidate path.
 * @param cwd - the session workspace root; empty leaves the path unchanged.
 * @returns the workspace-relative path, or the original.
 */
...
/**
 * One-line summary for a collapsed tool row: the first non-empty summary key
 * for that tool (web `deriveSummary`), falling back to the first line of the
 * result. Path-shaped summaries are made workspace-relative, matching what
 * the web shows.
 */
function toolSummary(item, cwd = '') {
  const args = parseToolArguments(item.arguments);
  const keys = TOOL_SUMMARY_KEYS[CODE_TOOLS.has(item.name) ? 'code' : item.name];
  const picked = firstString(args, keys ?? []);
  if (picked !== undefined) {
    const text = picked.split('\n')[0];
    return PATH_SUMMARY_TOOLS.has(item.name) ? relativizeToCwd(text, cwd) : text;
  }
  // Fallback: first line of the result, with ANSI codes stripped so that
  // displayWidth-based truncation (fit) and terminal rendering agree on the
  // column count — otherwise colour sequences would inflate the measured
  // width and cause the collapsed tool row to run past the conversation
  // block's edge.
  return stripAnsi(String(item.result ?? '').split('\n')[0]?.trim() ?? '');
}

/**
 * The one-line summary a collapsed bash row shows, mirroring the web's
 * terminal card: the tool's required `description` (the human-readable
 * "what this does" line, e.g. "Show working tree status"), never the raw
 * command. The command is only a fallback for a call that arrived without a
 * description, so the row always reads as a title rather than a shell dump.
 */
function bashRowSummary(item, cwd = '') {
  const args = parseToolArguments(item.arguments);
  const description = firstString(args, ['description']);
  if (description !== undefined) return description;
  return toolSummary(item, cwd);
}

/**
 * Normalize a tool call's arguments to an object. The harness passes bash /
 * read / web-search arguments as a live object, but a replayed older session
 * may carry the serialised JSON string; both shapes resolve here.
 */
function parseToolArguments(raw) {
  ...
  if (typeof raw === 'string' && raw.trim() !== '') {
    try {
      ...
      /* not JSON — treated as no extractable arguments */
    }
  }
  return null;
}

/** First non-empty string among `keys` on `args`, or undefined. */
...
/**
 * Render a tool call's arguments as display text for the expanded row. The
 * harness may hand over a live object (the common case) or a serialised JSON
 * string (older/replayed logs); both must read as JSON rather than
 * `[object Object]`.
 */
...
/**
 * Expanded body for a `code` variant tool (`run_code`): the program source
 * itself, never its JSON envelope — the web's `formatToolBody` special-cases
 * `code` the same way so the row reads as code rather than escaped newlines.
 * Falls back to the raw argument text when `code` is absent or unparsable.
 * @param raw - the call's arguments (live object or serialised JSON).
 * @returns the program body, or the raw argument text.
 */
function codeToolBody(raw) {
  const args = parseToolArguments(raw);
  if (args !== null) {
    const code = args.code;
    if (typeof code === 'string' && code !== '') return code;
  }
  return toolArgumentText(raw);
}
...
function conversationRows(
    items, width, showThinking, showFull, expandedThink = null, mode = 'plan',
    cwd = '') {
  const rows = [];
  ...
  const push = (bgColor, segments, text, copyable = true) => rows.push({
    bg: bgColor,
    segments,
  ...
  const pushText = (text, fgColor, bgColor) => {
    // Strip ANSI escape codes BEFORE wrapping so that `displayWidth` and
    // terminal rendering agree on every column — tool output frequently
    // carries colour sequences (\x1b[31m, \x1b[1m, …) that are zero-width
    // in the terminal but would otherwise inflate the measured width and
    // push rows past the conversation block's edge.
    const clean = stripAnsi(String(text ?? ''));
    for (const line of wrap(clean, width - 2)) {
      // Trim trailing whitespace so that extra spaces at the end of a line
      // never push the column past the measured width — which would leak
      // the row's background into the scrollbar column via the \x1b[K
      // erase at the end of each row.
  ...
  items.forEach((item, index) => {
    ...
    if (item.kind === 'user') {
      if (item.sourceKind !== undefined && item.sourceKind !== 'user') {
        ...
        const barColor =
            mode === 'goal' && index === items.length - 1 ? C.error : C.accent;
        rows.push(
            {bg: C.backgroundPanel, segments: [{text: '▌ ', fg: barColor}]});
      ...
      const thinkExpanded =
          thinkShown && (expandedThink?.has(item.id) ?? false);
      if (thinkShown) {
      ...
      if (item.interrupted)
        push(C.background, [{text: '  · stopped', fg: C.subtle}], '', false);
      ...
      const st = item.status === 'running' ?
          ...
          item.isError ? {glyph: '✖', fg: C.error, label: 'error'} :
      ...
      if (collapsed) {
        // Bash and Code mirror the web's neutral card: a plain title ("Bash" /
        // "Code") followed by the tool's human-readable summary, with NO
        // success checkmark and NO "done" label — the web's settled ok row
        // drops the state dot entirely. Only those two variants go quiet; the
        // generic rows keep their explicit glyph + label.
        const quiet =
            NEUTRAL_ROW_TOOLS.has(item.name) && item.status === 'done' &&
            !item.isError;
        // A blank glyph slot keeps every tool row's title on one left edge.
        const glyph = quiet ? '  ' : `${st.glyph} `;
        const summary = item.name === 'bash' ? bashRowSummary(item, cwd) :
                                                toolSummary(item, cwd);
        push(C.background, [
          {text: `  ▸  ${glyph}`, fg: quiet ? C.subtle : st.fg},
          {text: toolRowTitle(item.name), fg: C.text, bold: true},
          ...(quiet ? [] : [{text: ` · ${st.label}`, fg: C.muted}]),
          ...(summary ? [{
            text: `  ${fit(summary, Math.max(10, width - 26))}`,
            fg: C.muted
          }] :
                           []),
        ]);
      } else {
        const isCode = CODE_TOOLS.has(item.name);
        push(C.background, [
          {text: `  ▾  ${st.glyph} `, fg: st.fg},
          {text: toolRowTitle(item.name), fg: C.text, bold: true},
          {text: ` · ${st.label}`, fg: C.muted},
        ]);
        // A `code` row shows the program source, not its JSON envelope —
        // matching the web's `formatToolBody`, which unwraps `code` the same
        // way. A program is far longer than a typical argument blob, so the
        // collapsed body gets a wider budget before it is truncated.
        const argText = isCode ? codeToolBody(item.arguments) :
                                 toolArgumentText(item.arguments);
        const argMax = isCode ? 600 : 160;
        const args = item.arguments ? (showFull ? argText : truncate(argText, argMax)) :
                                      '';
        const result = item.result ?
            (showFull ? item.result : truncate(item.result, 400)) :
            '';
        if (args) pushText(args, C.muted, C.background);
        if (result)
          pushText(result, item.isError ? C.error : C.text, C.background);
      }
    } else if (item.kind === 'turn-error') {
      // turn-level LLM failure (web TurnErrorItem parity): collapsed to one
      // line by default; click the row to expand the full error text
      const expanded = expandedThink?.has(item.id) ?? false;
      thinkRows.set(rows.length, item.id);
      if (expanded) {
        push(C.background, [
        ...
        if (item.text) pushText(item.text, C.muted, C.background);
      } else {
        push(C.background, [
          {text: '  ▸ ', fg: C.subtle},
          {text: '⚠ ', fg: C.error},
          {text: 'turn error', fg: C.error, bold: true},
          ...(item.text ? [{
            text: `  ${fit(item.text, Math.max(10, width - 26))}`,
            fg: C.muted
          }] :
                          []),
        ]);
...
export function renderTui(controller, onExit, _stdout, _stdin) {
  ...
  if (!_stdout && !stdout.isTTY) {
    process.stderr.write(
        'deepcode: terminal UI requires a TTY (run inside a terminal)\n');
    onExit?.();
    return {quit: () => {}};
  }
  ...
  const updateTerminalTitle = () => {
    const title = String(controller.sessionTitle ?? '')
                      .replace(/[\u0000-\u001f\u007f\u0080-\u009f]/g, ' ')
                      .trim() ||
        'bash';
    if (title === lastTerminalTitle) return;
    lastTerminalTitle = title;
    stdout.write(`\u001b]0;${title}\u0007`);
  };
  ...
  // one row per message above the prompt box; completed items drop off
  let queue = [];
  let picker = null;
  ...
  let tuiMode = 'plan';
  // plan mode active state: tracks whether plan mode is actually enabled
  // (via /plan command). When disabled, the mode text is hidden entirely.
  let planActive = true;
  ...
  // conversation scroll offset: 0 = pinned to the bottom (latest); >0 =
  // scrolled up by that many conversation rows (mouse wheel / scrollbar thumb)
  let scrollOffset = 0;
  ...
  let sessionMetadataPromise = null;
  ...
  // History navigation state: index into past user messages (0 = most recent)
  // -1 means "not browsing history". When browsing, `inputSnapshot` holds the
  // draft text that was present before the first Up press so it can be
  // restored when the user presses Down past the last history entry.
  let historyIdx = -1;
  let inputSnapshot = '';
      ...
      if (picker === null) {
        picker = {
          ...
          onResolve: (opt) => {
            ...
            if (tok === null) return;
            input = input.slice(0, tok.start) + '@' + String(opt.value) +
                input.slice(tok.end);
          ...
          onCancel: () => {
              // keep the typed `@...` text; the user may continue typing or
              // delete it
          },
        };
        cmdIdx = 0;
  ...
  function rememberSessionMetadata(records) {
    for (const record of records)
      sessionMetadata.set(String(record.id), record);
  }

  function searchableSessionRecords() {
    return sessionListAll.map(
        (record) => sessionMetadata.get(String(record.id)) ?? record);
  }

  function currentSessionSearchContent() {
    return {
      items: sessionSearch?.items ?? [],
      hasMore: sessionSearch?.contentHasMore === true,
    };
  }

  function sessionSearchOptions(items) {
    return items.map((item) => {
      const id = String(item.id);
      const title = item.title || controller.titleFor(id) || id.slice(0, 20);
  ...
  function refreshSessionSearchResults(
      content = currentSessionSearchContent()) {
    if (picker === null || picker.kind !== 'session' || sessionSearch === null)
      return;
    const query = sessionSearch.query.trim();
    if (query === '') return;
    const merged =
        deriveSessionSearchResults(searchableSessionRecords(), content, query);
    sessionSearch.hasMore = merged.hasMore;
    picker.options = sessionSearchOptions(merged.items);
  }

  function warmSessionMetadata() {
    const records = sessionListAll;
    if (records.length === 0) return;
    if (sessionMetadataPromise?.records === records) return;
    const promise =
        controller.enrichTitles(records)
            .then((titled) => {
              if (sessionListAll !== records) return;
              rememberSessionMetadata(titled);
              if (picker?.kind === 'session' && sessionSearch !== null &&
                  sessionSearch.query.trim() !== '') {
                refreshSessionSearchResults();
                paint();
              }
            })
            .catch(() => {});
    sessionMetadataPromise = {records, promise};
  }
  ...
  /** Run (or refresh) the web-style metadata + full-text session search. */
  async function runSessionSearch(signal) {
    ...
    const contentPromise = controller.searchSessions(q, signal);
    warmSessionMetadata();
    const result = await contentPromise;
    if (signal !== undefined && signal.aborted)
      return;  // superseded mid-flight
    if (sessionSearch === null || sessionSearch.query.trim() !== q)
      return;  // stale
    sessionSearch.searching = false;
    sessionSearch.items = result.items;
    sessionSearch.contentHasMore = result.hasMore === true;
    refreshSessionSearchResults(
        {items: result.items, hasMore: sessionSearch.contentHasMore});
    cmdIdx = 0;
    paint();
  }
  ...
  function openProviderForm(route) {
    const existing = route !== null && route !== undefined ?
        controller.readProviderProfile(route) :
        undefined;
    const fields = [
      {
        key: 'route',
        label: 'provider',
        value: existing !== undefined ? route : '',
        secret: false
      },
      {
        key: 'displayName',
      ...
      },
      {
        key: 'api',
        label: 'api',
        value: existing?.api ?? 'openai-completions',
        secret: false
      },
      {
        key: 'baseURL',
      ...
      {key: 'apiKey', label: 'api key', value: '', secret: true},
      {
        ...
        value: (existing?.models ?? []).map((m) => m.id).join(', '),
        secret: false
      },
    ];
    pickerScroll = 0;
      ...
      onResolve: async () => {
        ...
        try {
          ...
          controller.addSystem(
              `Provider "${saved}" configured — pick it from /model.`);
        } catch (error) {
          controller.addSystem(`Failed to save provider: ${
  ...
  async function loadSessionPage() {
    if (sessionPage === null || sessionPage.loading ||
    ...
    const startLen = picker !== null && Array.isArray(picker.options) ?
        picker.options.length :
        0;
    const target = startLen + SESSION_PAGE_SIZE;
    while (sessionPage !== null &&
           sessionPage.loaded < sessionPage.all.length &&
           (picker?.options?.length ?? target) < target) {
      const next = sessionPage.all.slice(
          sessionPage.loaded, sessionPage.loaded + SESSION_PAGE_SIZE);
      ...
      if (picker !== null && picker.kind === 'session' &&
          typeof picker.buildOption === 'function') {
        picker.options.push(
            ...titled.filter((r) => r.blank !== true).map(picker.buildOption));
      }
    }
  ...
  let clickConv = null;
  // top area height of the last painted conversation frame (1 or 2 rows —
  // collapses on short terminals)
  let clickTopBarH = TOP_BAR_HEIGHT;
  ...
  // Initialize preset label from controller
  currentPresetLabel = controller.presetLabel || null;
  ...
  const off = controller.subscribe((snapshot) => {
    ...
    // The preset label lives on the controller (set at boot and on a switch),
    // so mirror it here rather than tracking a second copy that can drift.
    currentPresetLabel = controller.presetLabel || null;
    dirty = true;
  });

  controller.approvalHandler = (req) => new Promise((resolve) => {
    approval = {req, resolve};
    dirty = true;
    paint();
  });
  controller.questionsHandler = (request) => new Promise((resolve) => {
  ...
  // Command failures (e.g. a session that cannot be resumed) reach the user
  // as a transient error toast — the conversation area stays model-only.
  controller.notifyError = (message) => showToast(message, 'error');

  // One fast ticker drives three clocks: (1) the running spinner on the top
  // status row — a smooth ~15fps glyph spin (advancing one glyph per 530ms
  ...
  function renderInputContent(width, lineText, cursorAt = -1) {
    ...
    if (showCursor) {
      ...
      s += `${
          cursorOn ? `${bg(C.text)} ${RESET}${bg(C.backgroundElement)}` : ' '}`;
      s += fg(C.text) + visibleAfter;
    }
    const used = 2 + displayWidth(visibleBefore) +
        (showCursor ? 1 + displayWidth(visibleAfter) : 0);
    s += `${bg(C.backgroundElement)}${' '.repeat(Math.max(0, width - used))}${
        RESET}`;
    return s;
  }
  ...
  function renderPickerOverlay() {
    ...
    content.push({
      bg: bodyBg,
      segments: [{text: `  ${picker.title}`, fg: C.text, bold: true}]
    });
    // breathing room between the title bar and the search input (opencode
    // dialog padding)
    content.push({bg: bodyBg, segments: []});
    if (picker.kind === 'session' || picker.kind === 'file' ||
      ...
      const q = picker.kind === 'file' ? (matchMention(input)?.query ?? '') :
          picker.kind === 'session'    ? (sessionSearch?.query ?? '') :
                                         (picker.filter ?? '');
      const searching = picker.kind === 'session' ?
          sessionSearch?.searching === true :
          picker.loading === true;
      content.push({
        bg: bodyBg,
    ...
    if (picker.kind === 'provider-form') {
      ...
      picker.fields.forEach((f, i) => {
        ...
        const val = f.secret && f.value !== '' ?
            '•'.repeat(Math.min(12, f.value.length)) :
            f.value;
        content.push({
    ...
    if (cmdIdx >= pickerScroll + PICKER_MAX_ROWS)
      pickerScroll = cmdIdx - PICKER_MAX_ROWS + 1;
    if (pickerScroll > 0)
      content.push({bg: bodyBg, segments: [{text: '  ↑ more', fg: C.muted}]});
    ...
    const listOptions = picker.kind === 'provider' ?
        ...
        picker.options;
    const visible = picker.kind === 'provider-form' ?
        [] :
        listOptions.slice(pickerScroll, pickerScroll + PICKER_MAX_ROWS);
    ...
    if (picker.kind !== 'provider-form' && listOptions.length === 0) {
      const busy = (picker.kind === 'session' &&
                    (sessionSearch?.searching === true ||
                     (sessionPage !== null && sessionPage.loading))) ||
          picker.loading === true;
      if (!busy) {
        content.push({
          bg: bodyBg,
          segments: [{text: '  No results found', fg: C.muted}]
        });
      }
    }
    ...
    let more = picker.kind !== 'provider-form' &&
        pickerScroll + PICKER_MAX_ROWS < listOptions.length;
    if (picker.kind === 'session' && sessionSearch !== null)
      more = more || sessionSearch.hasMore;
    else if (picker.kind === 'session' && sessionPage !== null)
      more = more || sessionPage.loaded < sessionPage.all.length;
    if (more)
      content.push({bg: bodyBg, segments: [{text: '  ↓ more', fg: C.muted}]});
  ...
  function paint() {
    ...
    paintedRows = rows;
    const W = columns;
    // Full-width conversation with two columns of outer padding and one
    // scrollbar column, matching grok-build's top/status layout.
    const convW = Math.max(20, W - 4);

    const spinner = SPINNER[frame % SPINNER.length];
    const topBar = topBarRow(stats, W, status === 'running', spinner);
    ...
    const buildBottom = (width) => {
      const rows = [];
      // Pending inbox (grok-build queue pane): one row per queued message
      // directly above the prompt box — gray `#N` position prefix + the
      // first non-empty line in the user-message color, with a `(+N lines)`
      // suffix for multiline prompts. Finished messages are spliced out by
      // the agent, so they vanish on the next snapshot. Shown only while
      ...
      if (Array.isArray(queue) && queue.length > 0) {
            ...
            segments: [
              {text: `  #${i + 1} `, fg: C.muted},
              {
                text: fit(
                    firstLine,
                    Math.max(
                        8, width - 6 - String(i + 1).length - suffix.length)),
                fg: C.accent
              },
              ...(suffix !== '' ? [{text: suffix, fg: C.muted}] : []),
            ],
          });
        });
        if (queue.length > shown.length)
          rows.push({
            bg: C.background,
            segments: [{
              text: `  ↑ ${queue.length - shown.length} more`,
              fg: C.muted
            }],
          });
      }
      ...
      if (picker !== null && picker.kind !== 'file') {
        ...
        // top padding so the text does not touch the border
        rows.push({bg: C.backgroundPanel, segments: []});
        rows.push({
          bg: C.backgroundPanel,
          segments: [
            {
              text: ` ⚠ Allow ${approval.req.toolName}?`,
              fg: C.warning,
              bold: true
            },
            ...(approval.req.reason ?
                    [{text: ` — ${approval.req.reason}`, fg: C.muted}] :
                    []),
          ],
        });
        rows.push({
          bg: C.backgroundPanel,
          segments: [{text: '  [y] allow   [n] deny', fg: C.muted}]
        });
        // bottom padding so the text does not touch the border
        rows.push({bg: C.backgroundPanel, segments: []});
      } else {
        if (questions !== null) {
          ...
          const selected =
              Math.min(Math.max(qIdx, 0), Math.max(opts.length - 1, 0));
          // keep the selection inside the fixed-height option window
          if (selected < qScroll) qScroll = selected;
          if (selected >= qScroll + PICKER_MAX_ROWS)
            qScroll = selected - PICKER_MAX_ROWS + 1;
          const visible = opts.slice(qScroll, qScroll + PICKER_MAX_ROWS);
          // top padding so the question text does not touch the border
          rows.push({bg: C.backgroundPanel, segments: []});
          rows.push({
            bg: C.backgroundPanel,
            segments: [
              {text: ' ⚠ ', fg: C.warning},
              {
                text: fit(qtext, Math.max(12, width - 8)),
                fg: C.text,
                bold: true
              },
            ],
          ...
          if (visible.length === 0) {
            ...
            rows.push({
              bg: C.backgroundPanel,
              segments: [{
                text: '  (no options — enter to answer, esc to skip)',
                fg: C.muted
              }]
            });
          }
          visible.forEach((o, i) => {
            ...
            rows.push({
              bg: active ? tint(C.backgroundElement, C.accent, 0.12) :
                           C.backgroundPanel,
              segments: [
                {
                  text: `${active ? '  ›' : '   '} ${
                      typeof o?.label === 'string' ? o.label : ''}`,
                  fg: active ? C.accent : C.text,
                  bold: active
                },
              ],
            });
          });
          if (qScroll + PICKER_MAX_ROWS < opts.length)
            rows.push({
              bg: C.backgroundPanel,
              segments: [{text: '  ↓ more', fg: C.muted}]
            });
          rows.push({
            bg: C.backgroundPanel,
            segments: [{text: '  ↑↓ 选择 · enter 确认 · esc 跳过', fg: C.muted}]
          });
          // bottom padding so the hint text does not touch the border
          rows.push({bg: C.backgroundPanel, segments: []});
        }
        ...
        if (picker !== null && picker.kind === 'file') {
          ...
          if (q !== '' || picker.options.length > 0 || picker.loading) {
            ...
            const pickerScrollAdj = Math.max(
                0,
                Math.min(
                    cmdIdx, Math.max(0, listOptions.length - PICKER_MAX_ROWS)));
            const visible = listOptions.slice(
                pickerScrollAdj, pickerScrollAdj + PICKER_MAX_ROWS);
            if (pickerScrollAdj > 0)
              rows.push({
                bg: C.backgroundPanel,
                panel: true,
                segments: [{text: '   ↑ more', fg: C.muted}]
            ...
            visible.forEach((opt, i) => {
              ...
              rows.push({
                bg: sel ? tint(C.backgroundElement, C.accent, 0.12) :
                          C.backgroundPanel,
                panel: true,
                segments: segs,
              });
        ...
        if (input.startsWith('/') && cmdIdx >= 0) {
          ...
          const namePart =
              (space === -1 ? input : input.slice(0, space)).slice(1);
          const matches = matchedCommands(namePart);
          if (matches.length === 0) {
            rows.push({
              bg: C.backgroundPanel,
              panel: true,
              segments: [{text: '  no matching command', fg: C.muted}]
            });
          } else {
            matches.forEach(
                (c, i) => rows.push({
        ...
        rows.push({
          ...
          box: true,
          segments: [{text: '  ', fg: C.muted}]
        });
        // multi-line input: wrap source lines into a fixed physical viewport
        // (Ctrl+Enter inserts a line break); older content scrolls out while
        // the block cursor stays visible within the viewport
        ...
        const winStart = Math.max(
            0, Math.min(cursorLine, inputLines.length - MAX_INPUT_LINES));
        const shownLines =
            inputLines.slice(winStart, winStart + MAX_INPUT_LINES);
        let lineStart = 0;
        for (let k = 0; k < winStart; k += 1)
          lineStart += inputLines[k].length + 1;
        const wrappedRows = [];
        shownLines.forEach((line, i) => {
          const at = winStart + i === cursorLine ?
              Math.max(0, Math.min(cursor - lineStart, line.length)) :
              -1;
          ...
          wl.forEach((chunk, ci) => {
            ...
            if (winStart + i === cursorLine && at >= 0) {
              ...
              if (at >= charOfs &&
                  (at < charOfs + chunk.length ||
                   (ci === wl.length - 1 && at === charOfs + chunk.length))) {
                cur = at - charOfs;
              }
            }
            wrappedRows.push({line: chunk, cursorAt: cur});
            charOfs += chunk.length;
          });
          lineStart += line.length + 1;
        });
        // The viewport grows with the content up to MAX_INPUT_LINES (an empty
        // prompt stays at one compact row) and is measured in physical
        // wrapped rows, not source lines; otherwise moving the cursor to a
        // different set of source lines changes the prompt height when their
        // wrapping differs.
        const viewportRows =
            Math.max(1, Math.min(wrappedRows.length, MAX_INPUT_LINES));
        const cursorRow = wrappedRows.findIndex((row) => row.cursorAt >= 0);
        const rowStart = Math.max(
            0,
            Math.min(
                cursorRow < 0 ? 0 : cursorRow,
                wrappedRows.length - viewportRows));
        const visibleRows =
            wrappedRows.slice(rowStart, rowStart + viewportRows);
        while (visibleRows.length < viewportRows)
          visibleRows.push({line: '', cursorAt: -1});
        visibleRows.forEach(({line, cursorAt}) => {
          rows.push({
            bg: C.backgroundElement,
            box: true,
            inputRow: cursorAt >= 0,
        ...
        // each part separated by a "·" dot, uniformly. When plan/goal is
        // OFF (planActive is false), the mode text is hidden entirely.
        const modeText = planActive ? tuiMode : '';
        const modeFg = MODE_COLORS[tuiMode] ?? C.accent;
        const modelTxt = fit(controller.modelLabel, Math.max(8, width - 40));
        // build meta segments: [mode] · model · effort · permission · [preset]
        const metaSegs = [
          {text: '  ', fg: C.muted},
          ...(planActive ? [{text: modeText, fg: modeFg, bold: true}] : []),
          ...(planActive ? [{text: ' · ', fg: C.muted}] : []),
          {text: modelTxt, fg: C.text},
        ];
        const metaRest = fit(
            `${controller.effortLabel} · ${controller.permissionLabel}`,
            Math.max(
                8,
                width - displayWidth(modelTxt) -
                    (planActive ? displayWidth(modeText) + 3 : 0) -
                    (currentPresetLabel ? displayWidth(currentPresetLabel) + 6 :
                                          0) -
                    12));
        metaSegs.push({text: ' · ', fg: C.muted});
        metaSegs.push({text: metaRest, fg: C.muted});
          ...
          metaSegs.push({
            text: fit(
                ...
                Math.max(
                    6,
                    width - displayWidth(modelTxt) -
                        (planActive ? displayWidth(modeText) + 3 : 0) - 30)),
            fg: C.accent
          });
        }
        rows.push({
    ...
    if (items.length === 0) {
      ...
      const stackH =
          logoLines.length + 3 + floatRows.length + 1 + boxRows.length + 1;
      ...
      const floatRow = (f) => {
        const content = paintRow(
            f.bg ?? C.background, promptW, f.segments ?? [],
            f.bg ?? C.background);
        return bg(C.background) + ' '.repeat(promptLeft) + content +
            bg(C.background) + ' '.repeat(W - promptLeft - promptW) + RESET;
      };
      for (let i = 0; i < rows; i += 1) {
        ...
        if (afterTop >= 0 && afterTop < logoLines.length) {
          row = logoLines[afterTop];
            ...
            afterTop >= logoLines.length && afterTop < logoLines.length + 3) {
          // gap between the logo and the dialog
          row = paintRow(C.background, W, []);
        } else if (
            afterTop >= logoLines.length + 3 &&
            afterTop < logoLines.length + 3 + floatRows.length) {
          ...
          row = floatRow(f);
            ...
            afterTop >= logoLines.length + 3 + floatRows.length &&
            afterTop <
                logoLines.length + 3 + floatRows.length + boxRows.length) {
          ...
          const bar = fg(MODE_COLORS[tuiMode] ?? C.accent) +
              bg(C.backgroundElement) + '▌' + RESET + bg(C.background);
          if (isBottom) {
            ...
            const line =
                bg(C.backgroundElement) + ' '.repeat(promptW - 1) + RESET;
            row = bg(C.background) + ' '.repeat(promptLeft) + bar + line +
                bg(C.background) + ' '.repeat(W - promptLeft - promptW) + RESET;
          } else if (b.inputRow) {
            const box =
                renderInputContent(promptW - 1, b.line, b.cursorAt ?? -1);
            row = bg(C.background) + ' '.repeat(promptLeft) + bar + box +
                bg(C.background) + ' '.repeat(W - promptLeft - promptW) + RESET;
          } else {
            const box = paintRow(
                C.backgroundElement, promptW - 1, b.segments ?? [],
                C.backgroundElement);
            row = bg(C.background) + ' '.repeat(promptLeft) + bar + box +
                bg(C.background) + ' '.repeat(W - promptLeft - promptW) + RESET;
          }
          ...
          footerDone = true;
          const dir =
              fit(controller.cwd || process.cwd(), Math.max(10, promptW - 26));
          const left = `  ${dir}`;
          const leftW = displayWidth(left);
          const hintW = displayWidth('tab agents');
          const pad = Math.max(1, promptW - leftW - hintW);
          const footer = fg(C.muted) + left + ' '.repeat(pad) + fg(C.text) +
              'tab' + fg(C.muted) + ' agents';
          row = bg(C.background) + ' '.repeat(promptLeft) + footer +
              bg(C.background) + ' '.repeat(W - promptLeft - promptW) + RESET;
        } else {
          row = paintRow(C.background, W, []);
        }
    ...
    // ── conversation state: grok-build full-width agent layout ─────────────
    // top status row (shell tab label + cache/in/out/ctx metrics) + padded
    // conversation + prompt; no side panel — the session title rides on the
    // terminal window title instead.
    const conv = conversationRows(
        items, convW, showThinking, showFull, expandedThink, tuiMode,
        controller.cwd ?? '');
    const bottom = buildBottom(convW);

    // body rows hold the conversation below the top status area; the two
    // reserved gap rows keep the last message from touching the prompt box's
    // top border. The dialog footer (workspace dir left, "tab agents" right)
    // follows the prompt box directly, and one spare row below keeps the
    // dialog from hugging the window's bottom edge.
    const topBarH = topBarHeight(rows);
    const bodyHeight = Math.max(6, rows - bottom.length - 4 - topBarH);
    ...
    const thumbH = Math.max(
        1,
        Math.round(
            (bodyHeight * bodyHeight) / Math.max(totalConv, bodyHeight)));
    const thumbTop = maxScroll === 0 ?
        0 :
        Math.round(
            ((maxScroll - scrollOffset) / maxScroll) * (bodyHeight - thumbH));
    ...
    firstFrame = false;
    let footerDone = false;
    // the metrics ride on the middle row of the top status area
    const topMiddle = Math.floor(topBarH / 2);
    for (let i = 0; i < rows; i += 1) {
      // the top status area spans the full width (no padding/scrollbar
      // columns): a blank padding row, the status row, a blank gap row —
      // grok-build drops the padding and gap on short terminals
      if (i < topBarH) {
        const row = i === topMiddle ? topBar : paintRow(C.background, W, []);
        out += row + bg(C.background) + '\x1b[K' + RESET;
        if (i < rows - 1)
          out += '\r\n';
        else
          out += '\x1b[J';
      ...
      const bodyRow = i - topBarH;  // 0-based within the body area
      let left;
      if (bodyRow < bodyHeight) {
        const idx = start + bodyRow;
        ...
        if (b.boxBottom) {
          ...
          const box = paintRow(
              b.bg ?? C.backgroundElement, convW - 1, segs,
              b.bg ?? C.backgroundElement);
          left = bar + box;
        } else {
          left =
              paintRow(b.bg ?? C.background, convW, segs, b.bg ?? C.background);
        }
      } else if (!footerDone) {
        ...
        footerDone = true;
        const dir =
            fit(controller.cwd || process.cwd(), Math.max(10, convW - 26));
        const l = `  ${dir}`;
        const leftW = displayWidth(l);
        const hintW = displayWidth('tab agents');
        const pad = Math.max(1, convW - leftW - hintW);
        const footer = fg(C.muted) + l + ' '.repeat(pad) + fg(C.text) + 'tab' +
            fg(C.muted) + ' agents';
        left = bg(C.background) + footer;
      } else {
        left = paintRow(C.background, convW, [], C.background);
      }
      // scrollbar column at the right edge of the conversation (replaces one
      // column of the main column's right padding)
      let scrollCol;
      if (bodyRow < bodyHeight && showScrollbar) {
        const inThumb = bodyRow >= thumbTop && bodyRow < thumbTop + thumbH;
        scrollCol = bg(inThumb ? C.border : C.backgroundElement) + ' ' + RESET;
      } else {
        scrollCol = bg(C.background) + ' ' + RESET;
      }
      out +=
          bg(C.background) + '  ' + left + bg(C.background) + ' ' + scrollCol;
      // erase to end of line: set bg FIRST so the erase uses the correct
      // background color, not the terminal default (red); reset after it
      out += bg(C.background) + '\x1b[K' + RESET;
      if (i < rows - 1)
        out += '\r\n';
      else
        out += '\x1b[J';  // erase below the painted area on the last row
    }
    if (picker !== null) out += renderPickerOverlay();
  ...
  let pending = '';
  let bracketedPasteJustEnded = false;
  let plainTextBuffer = '';
  let plainTextEvents = 0;
  let plainTextTimer = null;
  const PLAIN_TEXT_INITIAL_DELAY_MS = 2;
  const PLAIN_TEXT_BURST_DELAY_MS = 10;
  const MAX_PLAIN_TEXT_EVENTS = 128;

  const flushPlainText = () => {
    if (plainTextTimer !== null) {
      clearTimeout(plainTextTimer);
      plainTextTimer = null;
    }
    if (plainTextBuffer === '') return;
    const text = plainTextBuffer;
    plainTextBuffer = '';
    plainTextEvents = 0;
    handlePastedText(text);
  };

  const queuePlainText = (text) => {
    plainTextBuffer += text;
    plainTextEvents += 1;
    if (plainTextEvents >= MAX_PLAIN_TEXT_EVENTS) {
      flushPlainText();
      return;
    }
    if (plainTextTimer !== null) clearTimeout(plainTextTimer);
    const delay = plainTextEvents === 1 ? PLAIN_TEXT_INITIAL_DELAY_MS :
                                          PLAIN_TEXT_BURST_DELAY_MS;
    plainTextTimer = setTimeout(flushPlainText, delay);
  };

  /**
   * Merged slash-menu: TUI-local commands + the live host catalog (web
   * parity).
   */
  function commandMenu() {
    const host =
        (controller.listCommands?.() ?? [])
            ...
            // /export is a browser-download command; /plan and /goal are
            // replaced by the Tab plan/goal mode toggle (which runs the same
            // /plan on|off command), so none of them are offered in the TUI.
  ...
  function matchedCommands(queryOverride) {
    const query =
        queryOverride ?? (input.startsWith('/') ? input.slice(1) : '');
    const scored =
        commandMenu()
  ...
  function handleClick(x, y) {
    ...
    const row = y - 1 - clickTopBarH;
    if (row < 0 || row >= clickBodyHeight) return;
    const convIdx = clickStart + row;
    const itemId = clickThinkRows?.get(convIdx);
    if (itemId !== undefined) {
      if (expandedThink.has(itemId))
        expandedThink.delete(itemId);
      else
        expandedThink.add(itemId);
      paint();
      return;
    }
  }
  ...
  /**
   * Extend the drag selection to the current cursor row (first motion shows
   * it).
   */
  ...
  function endDrag(x, y) {
    ...
    if (selection !== null && dragMoved && clickBodyHeight > 0) {
      ...
      const y1 = Math.max(clickTopBarH + 1, selection.y1);
      const y2 = Math.min(clickTopBarH + clickBodyHeight, selection.y2);
      const out = [];
      for (let row = y1; row <= y2; row += 1) {
        const convIdx = clickStart + (row - 1 - clickTopBarH);
  ...
  function renderToastOverlay(convW) {
    ...
    // just below the top status area (0-based) so the box never overlaps it
  ...
  function handlePastedText(text) {
    const pasted = normalizePasteText(text);
    if (pasted === '') return;
    if (picker !== null && picker.kind === 'session') {
      if (sessionSearch === null)
        sessionSearch = {
    ...
    if (picker !== null && picker.kind === 'provider') {
      picker.filter = String(picker.filter ?? '') + pasted;
      cmdIdx = 0;
      pickerScroll = 0;
      paint();
      return;
    ...
    if (picker !== null && picker.kind === 'provider-form') {
      const field = picker.fields[picker.fieldIdx];
      field.value = String(field.value ?? '') + pasted;
      paint();
      return;
    }
    input = input.slice(0, cursor) + pasted + input.slice(cursor);
  ...
  function handleKey(key) {
    ...
    if (picker !== null && picker.kind === 'session') {
      if (key === 'backspace' || key === 'space' ||
        ...
        if (key === 'backspace')
          sessionSearch.query = sessionSearch.query.slice(0, -1);
        else
          sessionSearch.query += key;
        cmdIdx = 0;
        scheduleSessionSearch();
        paint();
        return;
    ...
    if (picker !== null && picker.kind === 'provider') {
      if (key === 'backspace' || key === 'space' ||
          (key.length === 1 && !key.startsWith('\u001b'))) {
        if (key === 'backspace')
          picker.filter = String(picker.filter ?? '').slice(0, -1);
        else
          picker.filter = String(picker.filter ?? '') + key;
        cmdIdx = 0;
        pickerScroll = 0;
        paint();
        return;
    ...
    if (picker !== null && picker.kind === 'provider-form') {
      ...
      if (key === 'down') {
        picker.fieldIdx =
            Math.min(picker.fields.length - 1, picker.fieldIdx + 1);
        paint();
        return;
      }
      if (key === 'enter') {
    ...
    if (key === 'up') {
      ...
      if (picker !== null) {
        cmdIdx = Math.max(0, cmdIdx - 1);
          ...
          input.startsWith('/') && cmdIdx >= 0 &&
          matchedCommands().length > 0) {
        cmdIdx = Math.max(0, cmdIdx - 1);
      } else if (input.includes('\n') && historyIdx === -1) {
        // multi-line input: move cursor up one line, preserving column
        const inputLines = input.split('\n');
        let flat = 0;
        let cursorLine = 0;
        let cursorCol = 0;
        for (let k = 0; k < inputLines.length; k += 1) {
          const lineLen = inputLines[k].length;
          if (cursor <= flat + lineLen) {
            cursorLine = k;
            cursorCol = cursor - flat;
            break;
          }
          flat += lineLen + 1;
        }
        if (cursorLine > 0) {
          ...
          const prevLineStart = flat - inputLines[cursorLine - 1].length - 1;
          cursor = prevLineStart + Math.min(cursorCol, prevLine.length);
        }
      } else {
        // History navigation: gather user messages from the transcript
        ...
        if (userMessages.length > 0) {
          // Save the current draft on the first Up press
          if (historyIdx === -1) {
            inputSnapshot = input;
            historyIdx = 0;  // 0 = most recent user message
          } else {
            historyIdx = Math.min(historyIdx + 1, userMessages.length - 1);
          }
          input = userMessages[userMessages.length - 1 - historyIdx];
          cursor = input.length;
        }
      }
      paint();
    ...
    if (key === 'down') {
      if (questions !== null) {
        ...
        const opts = Array.isArray(questions.request.questions[0]?.options) ?
      ...
      if (picker !== null) {
        ...
        if (!(picker.kind === 'session' && sessionSearch !== null) &&
            ...
            picker.kind === 'session' && sessionPage !== null &&
            !sessionPage.loading && cmdIdx >= picker.options.length - 2 &&
            sessionPage.loaded < sessionPage.all.length) {
          void loadSessionPage();
        }
        const listLen = picker.kind === 'provider' ?
            picker.options
                .filter((opt) => {
                  const needle =
                      String(picker.filter ?? '').trim().toLowerCase();
                  if (needle === '') return true;
                  return String(opt.label).toLowerCase().includes(needle);
                })
                .length :
      ...
      } else if (input.startsWith('/') && matchedCommands().length > 0) {
        ...
        if (cmdIdx < 0)
          ...
          cmdIdx = Math.min(matchedCommands(namePart).length - 1, cmdIdx + 1);
      } else if (input.includes('\n') && historyIdx === -1) {
        // multi-line input: move cursor down one line, preserving column
        const inputLines = input.split('\n');
        let flat = 0;
        let cursorLine = 0;
        let cursorCol = 0;
        ...
        if (cursorLine < inputLines.length - 1) {
          const lineStart = cursor - cursorCol;
          const nextLineStart = lineStart + inputLines[cursorLine].length + 1;
          const nextLine = inputLines[cursorLine + 1];
          cursor = nextLineStart + Math.min(cursorCol, nextLine.length);
        }
      } else if (historyIdx >= 0) {
        // History navigation: move forward through history
        if (historyIdx === 0) {
    ...
    if (key === 'enter') {
      if (picker !== null) {
        ...
        const resolved = p.kind === 'provider' ? p.options.filter((opt) => {
      ...
      if (questions !== null) {
        ...
        const picked =
            opts[Math.min(Math.max(qIdx, 0), Math.max(opts.length - 1, 0))];
        q.resolve({
          answers: question ? [{
            id: question.id,
            selected: picked !== undefined && picked !== null ?
                [String(picked.label)] :
                []
          }] :
                              [],
      ...
      if (isCommandLine && text.startsWith('/') &&
        ...
        if (matches.length > 0) {
          const selected =
              matches[Math.min(Math.max(cmdIdx, 0), matches.length - 1)];
          input = '';
          cursor = 0;
          cmdIdx = 0;
          void runCommand(selected.id + args).catch((e) => {
            controller.addSystem(
      ...
      if (isCommandLine && text.startsWith('/')) {
        ...
        // Track whether plan mode is actually enabled (for color display)
        planActive = tuiMode === 'plan';
        void controller.executeCommand(planCmd).then(() => {
          // send() is async: it creates the session on this first message, then
          // delivers the text. Fire-and-forget — failures surface as a toast.
      ...
      input = '';
      cursor = 0;
      historyIdx = -1;  // reset history navigation
      paint();
      return;
    }
    if (key === 'escape') {
      // while the agent is responding, Esc interrupts the conversation
      if (status === 'running' && picker === null && approval === null &&
          questions === null) {
  ...
  function processPending() {
    while (running && pending.length > 0) {
      const bracketedPaste = consumeBracketedPaste(pending);
      if (bracketedPaste !== null) {
        flushPlainText();
        if (!bracketedPaste.complete) return;
        pending = bracketedPaste.rest;
        handlePastedText(bracketedPaste.text);
        bracketedPasteJustEnded = true;
        if (pending.startsWith('\r\n')) {
          pending = pending.slice(2);
          bracketedPasteJustEnded = false;
          handleKey('enter');
        }
        continue;
      }
      if (bracketedPasteJustEnded) {
        if (pending.startsWith('\r\n')) {
          pending = pending.slice(2);
          bracketedPasteJustEnded = false;
          handleKey('enter');
        ...
        if (pending[0] === '\r' && pending.length === 1) {
          holdForMore(10);
          return;
        }
        bracketedPasteJustEnded = false;
      }
      if (pending[0] === '\u001b' && isBracketedPasteStartPrefix(pending)) {
        flushPlainText();
        holdForMore();
        return;
      }
      if (pending[0] === '\u001b') {
        ...
        if (sgrMouse) {
          ...
          if (btn === 64)
            ...
            scrollBy(-3);
          else if (btn === 0 && sgrMouse[4] === 'M')
            beginDrag(x, y);  // press
          else if (btn === 32 && sgrMouse[4] === 'M')
            updateDrag(x, y);  // drag
          else if (btn === 0 && sgrMouse[4] === 'm')
            endDrag(x, y);  // release
      ...
      const allowLeadingTab = pending[0] === '\t' && pending.length > 1 &&
          plainTextBuffer !== '' && plainTextTimer !== null;
      const plainText = takePlainTextRun(pending, {allowLeadingTab});
      if (plainText.text !== '') {
        pending = plainText.rest;
        queuePlainText(plainText.text);
        continue;
      }
      flushPlainText();
      const ch = pending[0];
  ...
  async function runCommand(text) {
    if (text === '/model') {
      ...
      if (models.length === 0)
        return controller.addSystem('No models available.');
      ...
      picker = {
        ...
        options: models.map(
            (m) => ({
              label: `${m.model} · ${m.provider}${
                  cur && m.provider === cur.provider && m.model === cur.model ?
                      '  ● current' :
                      ''}`,
              value: m
            })),
        onResolve: async (opt) => {
          ...
          controller.addSystem(
              `Switched to ${opt.value.model} (${opt.value.provider}).`);
        },
        onCancel: () => {},
      };
      cmdIdx = 0;
    ...
    if (text.startsWith('/model ')) {
      ...
      const models = await controller.listModels();
      const m = models.find(
          (x) => x.model === arg || x.name === arg ||
              `${x.provider}/${x.model}` === arg);
      if (m === undefined)
        return controller.addSystem(`Unknown model "${arg}".`);
    ...
    if (text === '/reasoning') {
      ...
      if (efforts.length === 0)
        return controller.addSystem('No reasoning levels available.');
    ...
    if (text.startsWith('/reasoning ')) {
      ...
      const efforts = await controller.listEfforts();
      const e = efforts.find(
          (x) => x.id === arg || x.name.toLowerCase() === arg.toLowerCase());
      if (e === undefined)
        return controller.addSystem(`Unknown reasoning level "${arg}".`);
    ...
    if (text === '/preset') {
      ...
      if (presets.length === 0)
        return controller.addSystem('No agent presets available.');
      const cur = currentPresetLabel;
      pickerScroll = 0;
      picker = {
        kind: 'preset',
        ...
        options: presets.map(
            (p) => ({
              label: `${p.name}${p.id === cur ? '  ● current' : ''}`,
              value: p
            })),
        onResolve: async (opt) => {
          try {
            await controller.switchPreset(opt.value.id);
            currentPresetLabel = opt.value.name;
          } catch (e) {
            controller.addSystem(`Failed to switch preset: ${
    ...
    if (text.startsWith('/preset ')) {
      ...
      const p = presets.find(
          (x) => x.id === arg || x.name.toLowerCase() === arg.toLowerCase());
      if (p === undefined)
        return controller.addSystem(`Unknown preset "${arg}".`);
      try {
        await controller.switchPreset(p.id);
        currentPresetLabel = p.name;
      } catch (e) {
        return controller.addSystem(`Failed to switch preset: ${
            e instanceof Error ? e.message : String(e)}`);
      }
      paint();
      return;
    ...
    if (text === '/provider') {
      ...
      const options = [
        ...
        ...providers.map(
            (p) => ({
              label: `${p.displayName} (${p.route})${p.live ? '  ● live' : ''}${
                  p.configured ? '  ✓ configured' : ''}`,
              value: {
                route: p.route,
                displayName: p.displayName,
                existing: p.configured
              },
            })),
      ];
      pickerScroll = 0;
      picker = {
        ...
        options,
        onResolve: async (opt) => {
          if (opt.value.add === true)
            openProviderForm(null);
          else
            openProviderForm(opt.value.route);
        },
        onCancel: () => {},
      };
      cmdIdx = 0;
    ...
    if (text === '/session') {
      ...
      dlog('/session command received');
      const sessions = await controller.listSessions();
      dlog(`/session: ${sessions.length} sessions found`);
      ...
      picker = {
        ...
        options: [],
        buildOption: (s) => ({
          label: `${s.title || s.id.slice(0, 20)}${s.live ? '  ●' : ''}  ${
              fmt(s.createdAt)}`,
          value: s,
        }),
        onResolve: async (opt) => {
          // Save the search query before discarding the picker state, so it
          // can be sent as the first message to the resumed session.
          const pendingQuery = sessionSearch?.query?.trim() ?? '';
          sessionPage = null;
          sessionSearch = null;
          if (searchTimer !== null) {
            clearTimeout(searchTimer);
          ...
          searchAbort?.abort();
          const loaded = await controller.switchSession(opt.value.id);
          scrollOffset = 0;
          historyIdx = -1;  // reset history navigation
          // Only deliver the pending search text once the session actually
          // loaded — sending it after a failed resume would post it to the
          // session the user was already in.
          if (loaded && pendingQuery !== '') {
            void controller.send(pendingQuery);
          }
          paint();
        },
        onCancel: () => {
    ...
    if (text === '/permission') {
      ...
      if (perms.length === 0)
        return controller.addSystem('No permission presets available.');
      pickerScroll = 0;
      picker = {
        kind: 'permission',
        ...
        onResolve: (opt) => {
          void controller.switchPermission(opt.value.id);
    ...
    if (text.startsWith('/permission ')) {
      ...
      if (p === undefined)
        return controller.addSystem(`Unknown permission "${arg}".`);
      void controller.switchPermission(p.id);
    ...
    if (text === '/export' || text.startsWith('/export ')) {
      return controller.addSystem(
          `/export is a browser-only command and is not available in the TUI.`);
    }
  ...
  function quit() {
    ...
    escTimer = null;
    if (resizeRaf !== null) clearTimeout(resizeRaf);
    resizeRaf = null;
    if (plainTextTimer !== null) clearTimeout(plainTextTimer);
    plainTextTimer = null;
    plainTextBuffer = '';
    ...
    stdout.write(
        `\u001b[?1000l\u001b[?1002l\u001b[?1006l${BRACKETED_PASTE_DISABLE}`);
