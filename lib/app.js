        ...
        lines.forEach((line) => {
          rows.push({
            ...
            custom: (w, selBg) => {
              ...
              // selBg (opencode-style selection highlight) replaces the panel
              // fill while the row is drag-selected
              const innerW = textW + 2;
              const leftPad = Math.max(0, w - innerW - 1);
              const bubbleBg = selBg ?? C.backgroundPanel;
...
function sidePanelRows(statusText, running, stats, width, title = "", queue = null, queueOpen = false) {
  if (width < 12) return [];
  const W = width;
