...
function toolSummary(item) {
  ...
  if (typeof item.arguments === "object" && item.arguments !== null) {
    if (typeof item.arguments.command === "string" && item.arguments.command.trim() !== "")
      return item.arguments.command.trim();
    if (typeof item.arguments.code === "string" && item.arguments.code.trim() !== "")
      return item.arguments.code.trim();
    if (typeof item.arguments.query === "string" && item.arguments.query.trim() !== "")
  ...
  // width and cause the collapsed tool row to extend into the right panel.
 ...
 * click hit-testing.
 */
function conversationRows(items, width, showThinking, showFull, expandedThink = null, mode = "plan") {
  const rows = [];
  ...
  const pushText = (text, fgColor, bgColor) => {
    ...
    // cause the panel backgrounds to bleed across the conversation border.
  ...
  items.forEach((item, index) => {
    ...
    if (item.kind === "user") {
      if (item.sourceKind !== undefined && item.sourceKind !== "user") {
        ...
        const barColor = mode === "goal" && index === items.length - 1 ? C.error : C.accent;
        rows.push({ bg: C.backgroundPanel, segments: [{ text: "▌ ", fg: barColor }] });
      ...
      if (collapsed) {
        ...
        push(C.background, [
          ...
          // appears in the expanded view. Other tools keep showing their summary.
          ...(summary && item.name !== "bash"
            ? [{ text: `  ${fit(summary, Math.max(10, width - 26))}`, fg: C.muted }]
            : []),
        ]);
      } else {
        push(C.background, [
...
function sidePanelRows(running, spinner, stats, width, title = "", preset = null, queue = null, queueOpen = false) {
  if (width < 12) return [];
  const W = width;
...
export function renderTui(controller, onExit, _stdout, _stdin) {
  ...
  function paint() {
      ...
      if (picker !== null && picker.kind !== "file") {
        ...
        shownLines.forEach((line, i) => {
          ...
          wl.forEach((chunk, ci) => {
            ...
            // inputRow marks the line that renders the block cursor; it's the
            // line containing the cursor, not just the last line — otherwise
            // moving up in multi-line input hides the cursor
            rows.push({
        ...
        const metaRest = fit(`${controller.effortLabel} · ${controller.permissionLabel}`, Math.max(8, width - displayWidth(modelTxt) - displayWidth(modeText) - (currentPresetLabel ? displayWidth(currentPresetLabel) + 6 : 0) - 12));
        metaSegs.push({ text: " · ", fg: C.muted });
        metaSegs.push({ text: metaRest, fg: C.muted });
        if (currentPresetLabel) {
          metaSegs.push({ text: " · ", fg: C.muted });
          metaSegs.push({ text: fit(currentPresetLabel, Math.max(6, width - displayWidth(modelTxt) - displayWidth(modeText) - 30)), fg: C.accent });
        }
        rows.push({
          bg: C.backgroundElement,
          box: true,
    ...
    const conv = conversationRows(items, convW, showThinking, showFull, expandedThink, tuiMode);
    ...
    for (let i = 0; i < rows; i += 1) {
      ...
      out += bg(C.background) + "  " + left + bg(C.background) + " ".repeat(1 + convExtra) + scrollCol + right;
  ...
  function handleKey(key) {
    ...
    if (key === "up") {
      ...
      if (picker !== null) {
      ...
      } else if (input.startsWith("/") && cmdIdx >= 0 && matchedCommands().length > 0) {
        cmdIdx = Math.max(0, cmdIdx - 1);
        ...
        const userMessages = items
          .filter((item) => item.kind === "user" && typeof item.text === "string")
          .map((item) => item.text);
      ...
      if (isCommandLine && text.startsWith("/") && (text.length > 1 || cmdIdx >= 0)) {
        ...
        if (matches.length > 0) {
          ...
          void runCommand(selected.id + args).catch((e) => {
            controller.addSystem(`Error: ${e instanceof Error ? e.message : String(e)}`);
            paint();
          });
          paint();
          return;
  ...
  async function runCommand(text) {
      ...
      picker = {
        ...
        onResolve: async (opt) => {
          try {
            ...
            controller.addSystem(`Failed to switch preset: ${e instanceof Error ? e.message : String(e)}`);
          }
          paint();
        },
        onCancel: () => {},
    ...
    if (text.startsWith("/preset ")) {
      ...
      try {
        ...
        return controller.addSystem(`Failed to switch preset: ${e instanceof Error ? e.message : String(e)}`);
      }
      paint();
      return;
    }
