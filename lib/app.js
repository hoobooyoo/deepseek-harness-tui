...
function toolSummary(item) {
  // Most tools (bash, read, web-search, …) pass arguments as a live object
  // from the LLM, NOT as a JSON string — extract the command / code / query
  // directly so the collapsed summary shows the user's intent rather than
  // the first line of the (potentially huge) tool result.
  if (typeof item.arguments === 'object' && item.arguments !== null) {
    if (typeof item.arguments.command === 'string' &&
        item.arguments.command.trim() !== '')
      return item.arguments.command.trim();
    if (typeof item.arguments.code === 'string' &&
        item.arguments.code.trim() !== '')
      return item.arguments.code.trim();
    if (typeof item.arguments.query === 'string' &&
        item.arguments.query.trim() !== '')
      return item.arguments.query.trim();
  }
  // Legacy path where arguments were serialised as a JSON string.
    ...
    if (item.kind === 'user') {
      ...
      if (collapsed) {
        ...
        push(C.background, [
          ...
          // Bash: collapsed shows only "bash · done" — the full command only
          // appears in the expanded view. Other tools keep showing their
          // summary.
          ...(summary && item.name !== 'bash' ? [{
        ...
        const args = item.arguments ?
            (showFull ? item.arguments : truncate(item.arguments, 160)) :
            '';
