 ...
 *  In rc.2+ a namespace is a plain string key, not a `settingsNamespace()` brand. */
const PI_AI_NS = "llm-pi-ai";

/** Settings namespace the permission-preset picker writes its default into. */
const PERMISSION_SETTINGS_NS = "permission";

/** Settings namespace the agent-preset picker writes its default into. */
...
function blocksText(content) {
  ...
  // Strip ANSI escape codes so that coloured tool output doesn't corrupt the
  // transcript text used for copy/paste and display-width calculations.
  const strip = (s) => String(s).replace(/\u001b\[[\d;]*[a-zA-Z~$]/g, "");
  return content
...
function blocksReasoning(content) {
  ...
  return content
    ...
    .map((b) => strip(typeof b.text === "string" ? b.text : typeof b.content === "string" ? b.content : ""))
    .join("");
}
...
/**
 * Turn a resume failure into one short, actionable line. Persistence failures
 * are specific and recognizable, so they get a pointed explanation instead of
 * a raw stack-ish message; everything else passes through trimmed.
 * @param error - the thrown error from `agents.resume`.
 * @returns the user-facing message.
 */
function sessionResumeError(error) {
  const raw = error instanceof Error ? error.message : String(error);
  if (/already owned by an active write handle/.test(raw)) {
    return "Session is open in another dsh process (the web app holds a write lock) — close it there, then retry.";
  }
  if (/stored log is corrupt|seq gap/.test(raw)) {
    return "Session log is corrupt and cannot be replayed — start a new session.";
  }
  if (/invalid persisted inbox splice/.test(raw)) {
    return "Session log has an inconsistent inbox history and cannot be replayed.";
  }
  if (/not found/.test(raw)) {
    return "Session no longer exists in persistence.";
  }
  return `Could not load session: ${raw}`;
}
...
export class AgentController {
  ...
  /** In-flight lazy session creation, so concurrent callers share one agent. */
  #starting = null;
  ...
  /** Display name of the agent preset this session runs, or null when unknown. */
  presetLabel = null;
  ...
  /**
   * Report a one-line, user-visible error to the UI as a transient toast.
   * The conversation area stays model-only (`addSystem` is a deliberate
   * no-op), so command failures have no other way to reach the user — without
   * this they fail silently. Assigned by the UI layer, like the answerer slots.
   */
  notifyError = null; // (message) => void

  /**
   * Raise a user-visible error through whichever channel the UI installed:
   * the toast when present, otherwise nothing (headless smoke runs).
  ...
  #reportError(message) {
    ...
    if (text === "") return;
    dlog(`error: ${text}`);
    if (typeof this.notifyError === "function") this.notifyError(text);
  }
  ...
  #queueRows() {
    ...
    for (const target of ["next-step", "next-turn"]) {
      ...
      for (const message of list) {
        // Only user-sent messages enter the queue — tool-injected context
        // messages (source.kind !== "user") are internal and should not be
        // shown in the pending-items display.
        // The message stores `source` at the top level (set by createUserMessage),
        // NOT under `data`. Filter to only user-sent messages so that tool-injected
        // context messages are excluded from the pending-items display.
  ...
  /**
   * Read the model/preset/permission labels and reset the stats block WITHOUT
   * creating a session. Boot calls this so the TUI can render its empty home
   * (model, preset, permission, cwd) before any session exists — session
   * creation is deferred to the first user action (`ensureSession`).
   */
  async boot() {
    ...
    // The permission preset a brand-new session will adopt is the configured
    // default; the live `current(session)` read replaces it once one exists.
    const presets = this.#services.permissionPresets;
    if (presets !== undefined) this.permissionLabel = presets.defaultPreset;
    await this.#primePresetLabel();
    this.#emit();
  }

  /**
   * The preset a not-yet-created session will mount: the roster's configured
   * default, with its display name. Best-effort — an unreadable roster leaves
   * the label null rather than failing boot.
   */
  async #primePresetLabel() {
    ...
    try {
      ...
      this.presetLabel = found?.name ?? found?.id ?? id ?? null;
    } catch {
      this.presetLabel = null;
    }
  }
  ...
  /**
   * Create the session on first use and return its agent. This is the lazy
   * entry point every session-dependent action goes through: a plain TUI open
   * (no message, no command) never reaches it, so it leaves nothing on disk.
   *
   * Single-flight: concurrent callers (the plan-mode command racing the send
   * it precedes, a picker opening while a send is in flight) await the same
   * creation instead of minting two sessions. A failure is reported exactly
   * once per attempt — the shared promise rejects after reporting, so every
   * awaiting caller observes the same failure without a second toast.
   * @returns the live agent, or null when creation failed (already reported).
   */
  async ensureSession() {
    if (this.agent !== null) return this.agent;
    if (this.#starting === null) {
      this.#starting = this.#createAgent()
        .catch((error) => {
          this.#reportError(
    ...
    try {
      ...
      // Already reported by the shared attempt above.
      return null;
    }
  }
  ...
  async #createAgent() {
    ...
    const { agent, dispose } = await agents.create({
      ...
      setup: async (agentCtx) => {
        if (agentPresets !== undefined) {
          const preset = await agentPresets.mount(agentCtx);
          this.presetLabel = preset?.name ?? preset?.id ?? null;
        } else {
          this.presetLabel = null;
        }
    ...
    this.#lastSeq = agent.session.snapshotEvents().length;
    // Register this session with the shared workspace registry so the web
    // sidebar groups it under this workspace instead of "未分组". Fire-and-
    // forget: a registry failure must never break session creation (the
    // session itself is already live and usable).
    void this.#attachToWorkspace(String(agent.session.header?.id ?? ""));
    const presets = this.#services.permissionPresets;
    if (presets !== undefined) {
      this.permissionLabel = presets.current(agent.session) ?? "workspace-write";
    }
  ...
  /**
   * Start a brand-new session. Like a cold boot, this RESETS to the empty
   * home state without creating anything: the old agent is torn down (loop +
   * poll timer), the transcript and tool bookkeeping are wiped, the labels
   * fall back to their configured defaults, and session creation is deferred
   * again to the next real user action. `/new` therefore leaves no session on
   * disk until the user actually sends something.
   */
  async newSession() {
    this.#stopAgent();
    this.#transcript = [];
    ...
    this.sessionTitle = "";
    this.presetLabel = null;
    // boot() also resets the stats block and re-reads the configured labels.
    await this.boot();
  }
  ...
  async listSessions() {
    ...
    dlog(`listSessions: service=${query !== undefined}`);
    if (query === undefined) return [];
    try {
      const [records, root] = await Promise.all([query.listSessions(), this.#workspaceRoot()]);
      dlog(`listSessions: records=${records.length}, root=${root}`);
      ...
      dlog(`listSessions: filtered to ${out.length} sessions`);
      return out;
    } catch (err) {
      dlog(`listSessions: ERROR — ${err?.message}`);
      return [];
    }
  }
  ...
  /**
   * Attach one session to this workspace in the shared registry the web
   * sidebar reads. `workspaceRegistry.create` is idempotent per canonical
   * path (it returns the existing entity without retitling), so this both
   * creates the workspace on first use and joins the session to it.
   *
   * Failures are logged, never thrown: session creation must not fail because
   * a grouping side effect did. A missing registry (a composition without the
   * `workspace` row) is simply a no-op.
  ...
  async #attachToWorkspace(sessionId) {
    ...
    if (registry === undefined) {
      dlog(`attachWorkspace: no workspaceRegistry service; skipping ${sessionId}`);
      return;
    }
    try {
      const root = await this.#workspaceRoot();
      const workspace = await registry.create(root, basename(root));
      await workspace.attachSession(sessionId);
      dlog(`attachWorkspace: ${sessionId} joined '${root}'`);
    } catch (err) {
      dlog(`attachWorkspace: FAILED for ${sessionId} — ${err?.message}`);
    }
  }

  /**
   * Account for every stored session that belongs to an already-registered
   * workspace but is missing from that workspace's account. These are the
   * sessions the web sidebar renders under "未分组" — created before this
   * profile composed the `workspace` row, so no code path ever attached them.
   *
   * Only workspaces that already exist are repaired: a session whose canonical
   * cwd matches no registered workspace stays ungrouped, exactly as the web
   * treats it. A session already accounted for by any workspace is never moved
   * (the registry's one-owner invariant).
   *
   * Runs once per process after the tree settles, attaches only genuinely
   * unaccounted ids (idempotent and cheap on later boots), and never throws: a
   * failure leaves the sidebar exactly as it was.
   */
  async backfillWorkspaceMembership() {
    const registry = this.#services.workspaceRegistry;
    const query = this.#services.sessionQuery;
    if (registry === undefined || query === undefined) return;
    try {
      // Make sure this workspace is registered even on a first run, then index
      // every registered workspace by its canonical path.
      const root = await this.#workspaceRoot();
      await registry.create(root, basename(root));
      ...
      const attached = new Map(); // path → count, for one summary line
      const canonicalByCwd = new Map(); // cwd → canonical path, or null when it does not resolve
      for (const record of records) {
        const cwd = record.header.cwd;
        if (cwd === undefined) continue;
        const id = String(record.header.id);
        if (accounted.has(id)) continue;
        if (!canonicalByCwd.has(cwd)) {
          try {
            canonicalByCwd.set(cwd, await realpath(cwd));
          } catch {
            canonicalByCwd.set(cwd, null); // unresolvable cwd → stays ungrouped
          }
        }
        const canonical = canonicalByCwd.get(cwd);
        if (canonical === null) continue;
        const workspace = byPath.get(canonical);
        if (workspace === undefined) continue; // not a registered workspace
        try {
          await workspace.attachSession(id);
          accounted.add(id);
          attached.set(canonical, (attached.get(canonical) ?? 0) + 1);
        } catch (err) {
          dlog(`backfillWorkspace: FAILED for ${id} — ${err?.message}`);
        }
      }
      for (const [path, count] of attached) {
        dlog(`backfillWorkspace: attached ${count} session(s) to '${path}'`);
      }
    } catch (err) {
      dlog(`backfillWorkspace: ERROR — ${err?.message}`);
    }
  }
  ...
  async switchSession(sessionId) {
    ...
    // Resume the profile the session was RECORDED with, not the current
    // default: a session composed from 创造模式 (cordis) must come back as
    // 创造模式, even though this process booted on PTC 模式. This is what the
    // web's open path does (`composeAgent(presetForObservation(observation))`).
    const presetId = await this.#storedPresetId(sessionId);

    // Resume FIRST and tear the current agent down only on success: a failed
    // resume must leave the session the user is already in alive and usable,
    // rather than dropping them into a dead TUI with no agent.
    try {
      ...
      // the resume succeeded — now retire the previous agent
      this.#stopAgent();
      this.agent = agent;
      this.#disposeAgent = dispose;
      // Clear stale pending messages from the historical session's inbox so
      // they don't get claimed before the user's next message, and so they
      // don't appear in the queue display as pending items.
      agent.inbox.clear();
      ...
      this.sessionTitle = "";
      this.#lastSeq = 0;
      this.#fold(agent.session.snapshotEvents());
      this.#lastSeq = agent.session.snapshotEvents().length;
      ...
      // Resume failed: the current session stays live. Report the reason —
      // silently returning false is indistinguishable from "nothing happened"
      // and made an unloadable session look like a dead key binding.
      this.#reportError(sessionResumeError(error));
      return false;
    }
  }
  ...
  async searchSessions(query, signal) {
    ...
    dlog(`searchSessions: query="${query}", service=${queryService !== undefined}`);
    if (queryService === undefined || typeof queryService.searchSessions !== "function") {
      dlog(`searchSessions: ABORT — service unavailable`);
      return { items: [], hasMore: false };
    }
    try {
      // Mirror the web host's exact call pattern (dsh-host-apiproxy session.search):
      // no sessionFilters, eventFilters for type + surface, limit 20.
      ...
      dlog(`searchSessions: OK — ${items.length} items`);
      return {
        items: items.map((hit) => ({
      ...
      dlog(`searchSessions: ERROR — ${err?.message}\n${err?.stack}`);
      return { items: [], hasMore: false };
    }
  }
   ...
   *
   * Both providers are agent-scoped, so this materializes the session on
   * demand: opening the mention picker is an explicit user action asking for
   * session-scoped data.
   * @param query - the mention query after `@`.
   * @returns `{ files }` — workspace-relative file paths.
   */
  async searchFiles(query) {
    // Prefer the atFile service (dsh-at-file plugin) when available.
    // It walks the workspace with proper ignore rules and returns
    // structured results (relative paths, file kind). Fall back to the
    // glob tool from tool-fs-search.
    const q = String(query ?? "").trim();
    const agent = await this.ensureSession();
    if (agent === null) return { files: [] };
    try {
      const atFile = this.#services.atFile;
      if (atFile !== undefined) {
        const files = await atFile.search(agent, new AbortController().signal);
        if (q === "") return { files: files.map((f) => f.relative ?? f.path) };
        const needle = q.toLowerCase();
        const matched = files.filter((f) => {
          const name = f.relative ?? f.path;
          return name.toLowerCase().includes(needle);
        });
        return { files: matched.map((f) => f.relative ?? f.path).slice(0, 100) };
      }
      // fallback: glob tool
      const tools = this.#services.tools;
      if (tools === undefined) return { files: [] };
      const result = await tools.execute({
        callId: ToolCallId(`tui:glob:${randomUUID()}`),
        name: "glob",
        agent,
  ...
  /**
   * Send one user message. This is the action that creates the session: a TUI
   * that was merely opened holds no agent, so the message is what materializes
   * it. The text is delivered only after creation succeeds — a failed creation
   * reports an error instead of silently dropping the user's message.
   * @param text - the user's message.
   * @returns true when the message was handed to a live agent.
   */
  async send(text) {
  ...
  /**
   * List permission presets as `{ id, name, description, current }`. The table
   * itself is agent-independent, so this works before a session exists: with no
   * agent the "current" entry is the configured default a new session will
   * adopt, rather than forcing a session into existence just to render a menu.
   */
  listPermissions() {
    const presets = this.#services.permissionPresets;
    if (presets === undefined) return [];
    const current =
      this.agent === null ? presets.defaultPreset : presets.current(this.agent.session);
   ...
   * the web app exactly.
   *
   * The host catalog is scoped to a live agent, so it is EMPTY before a session
   * exists. This is called while painting, so it must never create one; the
   * menu still opens on its TUI-local commands, and running a host command
   * materializes the session through {@link executeCommand}.
   ...
   *
   * The host catalog is agent-scoped, so running a host command materializes
   * the session first — the user explicitly asked for something that needs one.
   ...
   *
   * Before a session exists the choice is recorded as the DEFAULT for future
   * sessions (the same `permission` settings section the web's picker writes),
   * so the session created by the next message adopts it. Selecting a preset
   * is never a reason to create a session.
   */
  async switchPermission(name) {
    const presets = this.#services.permissionPresets;
    ...
    if (this.agent === null) {
      ...
      if (settings === undefined) return undefined;
      try {
        await settings.replace(PERMISSION_SETTINGS_NS, { defaultPreset: name });
      } catch (error) {
        this.#reportError(
          `Could not save the permission default: ${error instanceof Error ? error.message : String(error)}`,
        );
        return undefined;
      }
  ...
  /** List agent presets as `{ id, name, description, current }`. */
  async listPresets() {
    const presets = this.#services.agentPresets;
    if (presets === undefined) return [];
    try {
      // Listing needs no live agent; the CURRENT preset comes from the
      // `agentPreset` session projection (the roster exposes no `current()`).
      const all = await presets.list();
      const current = this.agent === null ? undefined : this.#currentPresetId();
      return all.map((p) => ({
        id: p.id,
        name: p.name ?? p.id,
  ...
  /**
   * The preset id this session is composed from, read from the `agentPreset`
   * session projection (initialized from the session header, updated by
   * `agent-preset/selected`). Undefined when no session or no projection.
  ...
  #currentPresetId() {
    ...
    try {
      const value = projections.snapshot(this.agent.session).values.agentPreset;
  ...
  /**
   * The preset a PERSISTED session was recorded with, read from its replayed
   * projection snapshot without making it live. This is the pre-resume
   * counterpart of {@link #currentPresetId}: `switchSession` needs the stored
   * id to compose the resumed agent, which happens before any live agent
   * exists.
   *
   * Returns undefined when no observation is available (no sessionQuery, an
   * unreadable log, or a session that recorded no preset) — the caller then
   * falls back to the roster's configured default.
   * @param sessionId - the session about to be resumed.
   * @returns the recorded preset id, or undefined.
   */
  async #storedPresetId(sessionId) {
    const query = this.#services.sessionQuery;
    ...
    try {
      ...
      recorded = typeof value === "string" && value !== "" ? value : undefined;
    } catch (err) {
      // An unreadable log is reported by the resume itself, with a better
      // message; here it just means "no recorded preset to honour".
      dlog(`storedPreset(${sessionId}): observation failed — ${err?.message}`);
      return undefined;
    } finally {
      observation?.[Symbol.dispose]?.();
    }
    if (recorded === undefined) return undefined;
    // The recorded id may name a preset this deployment no longer ships (a
    // rename or deletion since the session was written). Falling back to the
    // roster default keeps the session loadable; failing here would make old
    // sessions permanently unopenable.
    const presets = this.#services.agentPresets;
    if (presets === undefined) return recorded;
    try {
      const roster = await presets.list();
      if (roster.some((p) => p.id === recorded)) return recorded;
      dlog(`storedPreset(${sessionId}): '${recorded}' is not in the roster; using the default`);
      return undefined;
    } catch (err) {
      dlog(`storedPreset(${sessionId}): roster read failed — ${err?.message}`);
      return recorded;
    }
  }

  /**
   * Switch the session's agent preset through the roster's own `select()`.
   *
   * That is the sanctioned mid-session switch: it re-links the live agent to
   * the chosen preset's standing composition, records the choice in the session
   * log, and refuses a session that is not blank (`agent-preset/locked`) —
   * silently recomposing the tools under an existing transcript would leave
   * logged tool calls the new composition cannot make. The refusal surfaces as
   * a plain message, so the TUI never dies on a user typo.
   *
   * With no session yet there is nothing to recompose, so the choice is stored
   * as the DEFAULT for future sessions (the `agent-presets` settings section the
   * web writes) and the session created by the next message mounts it. Picking
   * a preset is never itself a reason to create a session.
   * @param id - the preset id to compose the agent from.
   * @returns the preset now installed, or undefined when the switch was refused.
   */
  async switchPreset(id) {
    ...
    if (this.agent === null) {
      ...
      try {
        const roster = await presets.list();
        const picked = roster.find((p) => p.id === id);
        if (picked === undefined) {
          this.addSystem(`Unknown preset "${id}".`);
          return undefined;
        }
        if (settings === undefined) {
          this.addSystem("Preset settings are unavailable in this composition.");
          return undefined;
        }
        await settings.replace(PRESET_SETTINGS_NS, { default: id });
        this.presetLabel = picked.name ?? picked.id;
        this.#emit();
        return picked;
      } catch (error) {
        this.addSystem(
          `Could not select preset: ${error instanceof Error ? error.message : String(error)}`,
        );
        return undefined;
      }
    }
    let picked;
    try {
      const installed = await presets.select(this.agent, id);
      this.presetLabel = installed?.name ?? installed?.id ?? id;
      picked = installed;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      // A non-blank session (or any other refusal) keeps the old preset.
      this.addSystem(`Could not switch preset: ${message}`);
      return undefined;
    }
    // The tool set changed, so the next snapshot must reflect it immediately.
    this.#emit();
    return picked;
  }
