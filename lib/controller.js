...
export class AgentController {
  ...
  /** Switch the session's agent preset — tears down the current agent and
   * creates a fresh one with the chosen preset mounted. The transcript
   * and stats reset, exactly like /new. */
  async switchPreset(id) {
    const { agents, agentDefaultModel, agentPresets } = this.#services;
    if (agents === undefined || agentDefaultModel === undefined || agentPresets === undefined) return;
    const selection = agentDefaultModel.currentSelection();
    this.modelLabel = selection.model ?? selection.provider;
    this.effortLabel = selection.reasoningEffort ?? "low";
    this.#selectionRef = { current: selection, assembled: void 0 };

    // Save old agent state for rollback on failure
    const oldAgent = this.agent;
    const oldDispose = this.#disposeAgent;
    const oldTranscript = this.#transcript;
    const oldStreaming = this.#streaming;
    const oldToolIndex = this.#toolIndex;
    const oldSkipToolCards = this.#skipToolCards;
    const oldStats = this.#stats;
    const oldTitle = this.sessionTitle;
    const oldLastSeq = this.#lastSeq;
    const oldTimer = this.#timer;

    // Create new agent first, before tearing down the old one
    try {
      const { agent, dispose } = await agents.create({
        sessionId: SessionId(`deepcode-${randomUUID()}`),
        meta: { cwd: this.cwd || process.cwd() },
        agentOptions: { provider: selection.provider, model: selection.model },
        setup: async (agentCtx) => {
          if (agentPresets !== undefined) await agentPresets.mount(agentCtx, id);
          installModelSelection(agentCtx, this.#selectionRef);
        },
      });

      // New agent created successfully — now tear down the old one
      this.#stopAgent();
      this.#transcript = [];
      this.#streaming = null;
      this.#toolIndex.clear();
      ...
      // Rollback to old agent state on failure
      this.agent = oldAgent;
      this.#disposeAgent = oldDispose;
      this.#transcript = oldTranscript;
      this.#streaming = oldStreaming;
      this.#toolIndex = oldToolIndex;
      this.#skipToolCards = oldSkipToolCards;
      this.#stats = oldStats;
      this.sessionTitle = oldTitle;
      this.#lastSeq = oldLastSeq;
      this.#timer = oldTimer;
      this.addSystem(`Failed to switch preset: ${error instanceof Error ? error.message : String(error)}`);
      return;
    }
    ...
    try {
      ...
      this.presetLabel = picked?.name ?? id;
    } catch {
      this.presetLabel = id;
    }
